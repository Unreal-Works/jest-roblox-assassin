    ...
    executeLuau: async (luauScript, luauOptions) => {
        ...
        if (luauOptions.place.includes("demo_place.rbxl")) {
            ...
            const match = luauScript.match(
                /local jestOptions = HttpService:JSONDecode\((.*?)\)/s
            );
            if (match) {
...
describe("runJestRoblox.js", () => {
    ...
    it("should run tests with valid --place file", async () => {
        const requiredMatches = [
            ...
            " PASS  demo/src/__tests__/afolder/myCompartmentalizedTests.spec.ts",
            " PASS  demo/src/__tests__/div.spec.ts",
            " FAIL  demo/src/__tests__/mul.spec.ts",
            " PASS  demo/src/__tests__/myModule.spec.ts",
            " FAIL  demo/src/__tests__/sub.spec.luau",
            "fails when expecting 9 - 2 to equal 8",
            "src/__tests__/sub.spec.luau",
            " FAIL  demo/src/anothertestproject/ilovetesting.spec.ts",
            "src/anothertestproject/ilovetesting.spec.ts",
