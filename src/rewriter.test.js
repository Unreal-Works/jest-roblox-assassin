...
describe('ResultRewriter', () => {
                    ...
                    children: [
                        {
                            ...
                            children: [
                                {
                                    name: "test",
                                    filePaths: [path.join(mockOutDir, "test.luau")],
                                    children: []
                                }
                            ]
                        }
    ...
    describe('constructor', () => {
        ...
        it('should build module path map from sourcemap', () => {
            ...
            expect(rewriter.modulePathMap.has('ReplicatedStorage.src.test')).toBe(true);
        });
    });
    ...
    describe('datamodelPathToSourcePath', () => {
        it('should correctly remap nested paths with multiple separators', () => {
            const sourcePath = path.join(mockRootDir, '__tests__/afolder/myCompartmentalizedTests.spec.ts');
            rewriter.modulePathMap.set('ReplicatedStorage.src.__tests__.afolder.myCompartmentalizedTests.spec', {
                luauPath: path.join(mockOutDir, '__tests__/afolder/myCompartmentalizedTests.spec.luau'),
                sourcePath: sourcePath,
            });

            const result = rewriter.datamodelPathToSourcePath('__tests__/afolder/myCompartmentalizedTests.spec');
    ...
    describe('rewriteParsedResults', () => {
        it('should rewrite multiple test suites', () => {
            ...
            vi.spyOn(rewriter, 'formatPath').mockReturnValue('formatted/path');
            vi.spyOn(rewriter, 'datamodelPathToSourcePath').mockReturnValue('/source/path');
            vi.spyOn(rewriter, 'rewriteStackString').mockImplementation((str) => `rewritten ${str}`);
            vi.spyOn(rewriter, 'parseFrame').mockReturnValue(null); // No frame for simplicity
            vi.spyOn(rewriter, 'formatFailureMessage').mockImplementation((str) => `formatted ${str}`);

            rewriter.rewriteParsedResults(results);

            expect(rewriter.formatPath).toHaveBeenCalledTimes(2);
            expect(rewriter.datamodelPathToSourcePath).toHaveBeenCalledWith('test1');
            expect(rewriter.datamodelPathToSourcePath).toHaveBeenCalledWith('test2');
